...
uniform mat4 uZoomBox;
uniform bool uUseZoomBox;
uniform bool uShowLabeledOnly;

attribute float initialClassification;
...
void doClipping(){
	...
	if(uShowLabeledOnly){
		if( abs(classification - initialClassification) < 0.1 ){
			gl_Position = vec4(100.0, 100.0, 100.0, 0.0);
			return;
		}
	}
	...
	if(uUseZoomBox){
		vec4 clipPosition = uZoomBox * modelMatrix * vec4( position, 1.0 );
