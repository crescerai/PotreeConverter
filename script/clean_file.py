...
def need_cleaning(file_path):
    """
    Checks if a LAS/LAZ file requires cleaning.

    This function uses the existing `load_las` function to load the file
    and checks for issues such as NaN values, presence of `uint64` or `int64` columns,
    or any other conditions that indicate the file needs cleaning.

    Args:
        file_path (str or Path): Path to the LAS/LAZ file.

    Returns:
        bool: True if the file requires cleaning, otherwise False.
    """
    try:
        # Load the file to inspect its data
        las = lp.read(file_path)

        # Check for NaN values in the file
        df = load_las(file_path)
        if df.isnull().any().any():
            return True

        # Check for int64 or uint64 columns
        int64_columns = [
            dim for dim, dtype in las.point_format.dimension_types.items()
        ...
        if int64_columns or uint64_columns:
            return True

        # Add additional checks here as needed (e.g., unsupported dimensions)
        # Uncomment and modify based on your cleaning logic:
        # unsupported_columns = [col for col in df.columns if col not in expected_columns]
        # if unsupported_columns:
        #     return True

        # If no issues are found, the file does not need cleaning
        return False

    except Exception as e:
        print(f"Error while checking file {file_path}: {e}")
        return True  # If an error occurs, assume the file needs cleaning


# if __name__ == "__main__":
#     try:

#         DATA_DIR = Path("/home/sachin/point-cloud-segmentation/data/iris_data/nevvis/predictions")
#         all_las = list(DATA_DIR.rglob("*.las")) + list(DATA_DIR.rglob("*.laz"))
#         ic(all_las)
#         for file_loc in tqdm.tqdm(all_las):
#             df = load_las(file_loc)
#             create_las(df, file_loc, file_loc)
